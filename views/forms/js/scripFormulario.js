...
$("#btn_guardar").click(function (e) {
  e.preventDefault(); 
    ...
    if (campos.txtEmail==true && campos.txtApePaterno==true && campos.txtApeMaterno==true && 
      campos.txtnombres==true && campos.txtNumCelular==true && campos.CboPais==true /* && 
      campos.respuesta6==true && campos.respuesta7 */) {
    ...
    success: function (resp) {
      ...
      datos.forEach((element) => {
        ...
        Cursos+=`<div class="contenChecks" capIdPregunta="${element.id}" capIDRespuesta="${ele.idResp}">
                   <input type="radio" name="Respuesta${i}" class="cheks" id="${ele.idResp}" value="${ele.idResp}" required> 
...
function validarRadios(idPregunta,idrespuesta){
  if($(`input:radio[id=${idrespuesta}]:checked`).val()==undefined){
    ...
    document.querySelector(`#grupo__${idPregunta} #${element.id}`).classList.remove('fa-check-circle');
    document.querySelector(`#grupo__${idPregunta} #${element.id}`).classList.add('fa-times-circle');
